...
sav=[];
while(rn>=r1/10^2)
unp1_1=(eye(N(1))+inv(HH/h1)*B{1})*un - inv(HH/h1)*f;
unp1_2=(eye(N(1))+inv(HH/h2)*B{1})*unp1_1 - inv(HH/h2)*f;
unp1_3=(eye(N(1))+inv(HH/h3)*B{1})*unp1_2 - inv(HH/h3)*f;
unp1=basic_iters*unp1_3-basic_iters*inv(B{1})*f+inv(B{1})*f;
un=unp1;
iter=iter+1;
rn=norm(B{1}*unp1-f);
sav=[sav [iter;rn]];
...
D = real(diag(D));
ans_con_rate=max(D)
Theoretical_con_rate=cos(pi/(N(1)+1))*h1
