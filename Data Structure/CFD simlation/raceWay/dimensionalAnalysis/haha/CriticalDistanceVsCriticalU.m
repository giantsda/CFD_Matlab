...
for c=1:1
    ...
    for n=1:length(UcriticalArray)
        ...
        % axis equal;
        A = sortrows(Data.mesh(i,1:2),[1 2]); % removing overlapping points
        B=A(1,1:2);% project to XY plan
        for i=2:length(A)
        ...
        imshow(rot90(1-D))
        hold on;
        criticalDistance(n)=xMesh(max(Boun{n,1}(:,1)))-xmin;
