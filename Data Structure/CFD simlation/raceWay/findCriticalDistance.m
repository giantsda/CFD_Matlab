...
for i=2:2
    cd(num2str(i))
    fprintf(fid1,"case %d:",i);
    Ucritical=UcriticalArray(i);
    distanceArray=[];
%     if mod(i,8)==1
...
%     end
    ...
    for i=1:1%length(times)
        ...
        U(i,:)=[];
        vof(i,:)=[];
        i=find(mesh(:,3)>0.19); % remove the nodes that is adjcent to air. Data here will have close to 1 vof but hugh velocity.
        mesh(i,:)=[];
        U(i,:)=[];
        vof(i,:)=[];
        i=find(U>Ucritical);
        mesh(:,3)=[]; % project to XY plane
        scatter(mesh(i,1),mesh(i,2),2,'b','filled')
        axis equal;
        A = sortrows(mesh(i,:),[1 2]); % removing overlapping points
        B=A(1,:);
        for i=2:length(A)
        ...
        imshow(rot90(1-D))
        hold on;
        distanceArray=[distanceArray xMesh(max(Boun{n,1}(:,1)))-xmin];
        plot(Boun{n}(:,1), size(D,2)-Boun{n}(:,2)+1, 'r', 'LineWidth', 1)
        hold off;
        title(['criticalDistance=' num2str(xMesh(max(Boun{n,1}(:,1)))-xmin)]);
        pause();
        saveas(gcf,['../criticalDistance-' time '.png']);
        cd("../")
    end
    distance=mean(distanceArray);
