...
plot([min(Boun(:,1)); i] ,[floor(r/2); floor(r/2)],'b','LineWidth',2);
plot([8*r;8*r],[0;r],'b','LineWidth',2);
criticalDistance=(xMesh(min(Boun(:,1)))-min(Data.mesh(:,1)))/radius;
hold off;
title(['Ucritical=' num2str(Ucritical) 'criticalDistance=' num2str(criticalDistance,'%10.2f') '*Radius']);
 
saveas(gcf,['D:\CFD_second_HHD\06232021\204\Data\Case_' num2str(caseN) '.png'])
end
