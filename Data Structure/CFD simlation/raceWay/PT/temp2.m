...
x0 =0.310953489990063
y0 =  0.231935685619717

while dx>1e-15   % when dx is larger than the machine error
    %     dx
    if PICK
        %         [maxvalues, ind] = maxk(gradientMag(:), 100); % find n largest
        [A,ind]=sort( gradientMag(:),'descend');  % Before Matlab 2017b , there is no maxk function.
        ind=ind(1:1000);
...
%     colormap parula
    ...
    axis equal;
    axis tight
    Frame(frame)=getframe(gcf); %% store frame
    frame=frame+1;
    zm=zm+5;
    pause(0);
end
