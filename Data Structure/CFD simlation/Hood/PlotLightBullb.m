function [vertex face]= PlotLightBullb(filename,color)
% close all;
% filename='E:\desktop\temp\lightBulb.vtk';
% nargin=2;
%  color=1
...
for i=start:last
    ...
    if nargin==1
        h=patch('Faces',face(number(1,i):number(2,i),2:1+i),'Vertices',vertex,'FaceColor','none','EdgeColor','b');
         ...
         h=patch('Faces',face(number(1,i):number(2,i),2:1+i),'Vertices',vertex,'FaceColor',[0.2 0.2 0.2],'EdgeColor','none');
    end 
    hold on;
end
    lightangle(0,90)
    ...
    h.AmbientStrength = 1;
    h.DiffuseStrength = 1;
    h.SpecularStrength = color;
    h.SpecularExponent = 0.1;
