...
% define the particle number, spatial limits
 ...
 particle_number=100000; % define the particle number 
x=-60/100  % I think the units is meter here for FUME
y=5.1/100
z=120/100

% x=76/100  % I think the units is meter here. for NANO
% y=15/100
% z=5.1/100
...
for k=1:elen_z     %generate layer by layer
    ...
    for j=1:  elen_y
        for i=1:elen_x
            ...
            if (cell_x-center(1))^2+(cell_y-center(2))^2+(cell_z-center(3))^2<=(5/100)^2
...
ddd=d;
fprintf('Total number of particles used: %d \n', d);
...
ylabel('y')
zlabel('z')
title(['initial number is  ' num2str(elen_x*elen_y*elen_z)])
hold off;
...
for i=1:ddd-1 %assign xyz postion to numberpart
...
writeinfo{1}=data{1};
writeinfo{2}=data{2};
filename1=[num2str(ddd) '_' num2str(d) '.inj'];
fid=fopen(['E:\desktop\temp\' filename1],'wt');
disp(['generating E:\desktop\temp\' filename1 '....']);
for i=1:length(writeinfo)
    writ=writeinfo{i};
    fprintf(fid,'%s\n',writ);
end
fclose(fid);
